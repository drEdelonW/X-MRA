...
void PCA9685::setFreq_Hz(Hertz freq) {
    ...
    _iEP.RegWrite(MODE1, (old_mode & 0x7F) | SLEEP_BIT);   usleep(500); // Wait for oscillator to stop
    float prescale_val = Hz(OSC_FREQ_HZ) / (MAX_VAL * freq) - 1.f;
    _iEP.RegWrite(PRE_SCALE, floorf(prescale_val + 0.5f));    // Set prescale
    _iEP.RegWrite(MODE1, old_mode);  /* Exit sleep */      usleep(500); // Allow oscillator to stabilize
    _iEP.RegWrite(MODE1, old_mode | 0x80); // Restart (if auto-increment etc. is needed)
}
