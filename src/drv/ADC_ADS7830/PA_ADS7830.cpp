...
static uint8_t _muxCode(uint8_t channel) {
    // ADS7830 mux select is non-sequential (datasheet Table 1):
    // select code 0..7 -> physical channel 0,4,1,5,2,6,3,7
    static const uint8_t remap[8] = {0, 4, 1, 5, 2, 6, 3, 7};
    return remap[channel & 0x7];
}

uint8_t ADS7830::readChannel(uint8_t channel) {
    uint8_t value = 0;
    uint8_t cmd = 0x84 | (_muxCode(channel) << 4); // SD=1 single-ended, PD1:PD0=11 ref+ADC on
