...
static const uint64_t keyTable[KEY_COUNT] = {
    ...
    [KEY_F2]  = SEQ5(0x1B, 0x5B, 0x31, 0x32, 0x7E),/* ESC [ 13 ~ */
...
static uint64_t _keyBuff;
bool isKeyPressed() {
    _keyBuff = 0;

    uint8_p base = (uint8_p)&_keyBuff;   /* first byte of the word */
    uint8_p ptr = base;
    uint8_p end = base + sizeof(_keyBuff);
