...
bool Triangle3D::boundingBoxOverlap(const Triangle3D& other) const {
    ...
    t1_min = t1_max = vertices[0].x;
    t2_min = t2_max = other.vertices[0].x;

    for (int idx = 1; idx < TRI_NUM; ++idx) {
        if (vertices[idx].x < t1_min) {
            t1_min = vertices[idx].x;
        } else if (vertices[idx].x > t1_max) {
            t1_max = vertices[idx].x;
        }

        if (other.vertices[idx].x < t2_min) {
            t2_min = other.vertices[idx].x;
        } else if (other.vertices[idx].x > t2_max)
            t2_max = other.vertices[idx].x;
    }
    ...
    t1_min = t1_max = vertices[0].y;
    t2_min = t2_max = other.vertices[0].y;

    for (int idx = 1; idx < TRI_NUM; ++idx) {
        if (vertices[idx].y < t1_min) {
            t1_min = vertices[idx].y;
        } else if (vertices[idx].y > t1_max) {
            t1_max = vertices[idx].y;
        }

        if (other.vertices[idx].y < t2_min) {
            t2_min = other.vertices[idx].y;
        } else if (other.vertices[idx].y > t2_max) {
            t2_max = other.vertices[idx].y;
        }
    }
    ...
    t1_min = t1_max = vertices[0].z;
    t2_min = t2_max = other.vertices[0].z;

    for (int idx = 1; idx < TRI_NUM; ++idx) {
        if (vertices[idx].z < t1_min) {
            t1_min = vertices[idx].z;
        } else if (vertices[idx].z > t1_max) {
            t1_max = vertices[idx].z;
        }

        if (other.vertices[idx].z < t2_min) {
            t2_min = other.vertices[idx].z;
        } else if (other.vertices[idx].z > t2_max) {
            t2_max = other.vertices[idx].z;
        }
    }
...
bool Triangle3D::isPointInProjectionZ(const Vector3D& point) const {
    ...
    float fullArea = 0.5f * abs(x1 * (y2 - y3) + x2 * (y3 - y1) + x3 * (y1 - y2));

    // Calculate areas with the point
    float area1 = 0.5f * abs(px * (y2 - y3) + x2 * (y3 - py) + x3 * (py - y2));
    float area2 = 0.5f * abs(x1 * (py - y3) + px * (y3 - y1) + x3 * (y1 - py));
    float area3 = 0.5f * abs(x1 * (y2 - py) + x2 * (py - y1) + px * (y1 - y2));

    // If the sum of areas equals the full area, the point lies within or on the boundary
    float sumAreas = area1 + area2 + area3;
    return (sumAreas >= fullArea - 1e-5f) && (sumAreas <= fullArea + 1e-5f);
}
